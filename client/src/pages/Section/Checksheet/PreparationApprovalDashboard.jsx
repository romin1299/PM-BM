...
const PreparationApprovalDashboard = () => {
  const context = useContext(RoutingContext);
  const [tableData, setTableData] = useState([]);
  const navigate = useNavigate();

  const getApprovalRequestDataForPreparationPhase = async () => {
    try {
      const res = await fetch("/getApprovalRequestDataForPreparationPhase", {
        method: "GET",
        headers: {
  ...
  const preparationApprovalDataOfCSV = [
    {
      label: "Cell Name",
  ...
  const downloadPDFForPreparationApprovalData = () => {
    const doc = new jsPDF();
    let rows = [];
    ...
    doc.text(`Preparation Approval Data`, 15, 10);
    ...
    doc.save(`Preparation_Approval_Data_${timeStamp()}`);
  };
  ...
  const actions = [
    ...
    },
    {
      ...
      isFreeAction: true,
      onClick: (event) => {
        downloadPDFForPreparationApprovalData();
      },
    },

    {
      icon: () => (
        <CSVLink
          headers={preparationApprovalDataOfCSV}
          className="downloadCSV text-decoration-none"
          data={tableData ? tableData : []}
          filename={`Preparation_Approval_Data_${timeStamp()}`}
      ...
      <div className="pageCard">
        <div className="creationDashboard">
          <h4 style={{ padding: "1rem 0 0 1rem" }}>
            Preparation Approval Dashboard
          </h4>
...
export default PreparationApprovalDashboard;
