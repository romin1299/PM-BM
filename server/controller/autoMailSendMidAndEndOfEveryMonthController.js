...
cron.schedule(
  ...
  async (req, res) => {
    try {
      console.log(
        "...........Send mail at 015st and End of every Month..........."
      );
      ...
      for (let i = 0; i < sectionInfo?.length; i++) {
        if (sectionInfo[i].dashboardLevel === "Yes") {
          ...
          monthlyChartDataOfSummery = await Machine.populate(
            monthlyChartDataOfSummery,
            {
              path: "line_names",
          ...
          const planVsActualTableBodyMappingFunction = (item, index) => {
            return `<tr>
                            ...
                            <td style="border: 1px solid black;text-align: left;padding: 8px;">${
                              item?.chartData?.sumVariableForTotalCompleted
                            }</td>
                            <td style="border: 1px solid black;text-align: left;padding: 8px;">${
                              item?.chartData?.sumVariableForTotalOngoing
                            }</td>
                            <td style="border: 1px solid black;text-align: left;padding: 8px;">${
                              item?.chartData?.sumVariableForTotalSchedule -
                              item?.chartData?.sumVariableForTotalCompleted -
          ...
          planVsActualTable = `<table style="font-family: arial, sans-serif;border-collapse: collapse;width: 100%;">
                        ...
                        ${await monthlyChartDataOfSummery
                          ?.map((item, index) =>
                            planVsActualTableBodyMappingFunction(item, index)
                          )
                          ?.join("")}
                        </table>`;
          ...
          currentMonthTable = `<table style="font-family: arial, sans-serif;border-collapse: collapse;width: 100%;">
                        ...
                        ${await machineDataForCurrentMonth
                          ?.map((item, index) =>
                            currentMonthTableBodyMappingFunction(item, index)
                          )
                          ?.join("")}
                        </table>`;
          ...
          machineDataForPreviousMonth.map((keyForCheckSheet) => {
            for (
              let i = 0;
              i <
              Object.keys(keyForCheckSheet?.checkSheet_data?.PMStatus)?.length;
              i++
            ) {
              ...
              if (
                keyForCheckSheet?.checkSheet_data?.PMStatus[month] ===
                  "Done with delay" &&
                keyForCheckSheet?.checkSheet_data
                  ?.flagOfDoneWithDelayForOneMonth?.[month] ===
              ...
              if (
                ...
                skipMachineDataWithEveryMonth.push(
                  new Object({
                    ...
                    PMStatus:
                      keyForCheckSheet?.checkSheet_data?.PMStatus[
                        previousMonth
                      ],
                    checkSheet_data: keyForCheckSheet?.checkSheet_data,
                  })
          ...
          previousMonthTable = `<table style="font-family: arial, sans-serif;border-collapse: collapse;width: 100%;">
                        ...
                        ${await machineDataForPreviousMonth
                          ?.map((item, index) =>
                            previousMonthTableBodyMappingFunction(item, index)
                          )
                          ?.join("")}
                        </table>`;
          ...
          for (
            let subIterator = 0;
            subIterator < subSectionsData?.length;
            subIterator++
          ) {
            ...
            cellData = await Cell.find({
              subSection_names: subSectionsData[subIterator]._id,
            }).sort({ cell_sequence: 1 });
            ...
            machineDataForPreviousMonth.map((keyForCheckSheet) => {
              for (
                let i = 0;
                i <
                Object.keys(keyForCheckSheet?.checkSheet_data?.PMStatus)
                  ?.length;
                i++
              ) {
                let month = financialYearWiseMonthKeyArray[i];
      ...
      subSectionsData = await SubSection.find({
        section_names: { $in: sectionInfo?.map((item) => item._id) },
      }).sort({ subSection_sequence: 1 });
    } catch (error) {
