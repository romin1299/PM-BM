...
router.post("/postSubSectionToGetCellList", authenticate, async (req, res) => {
  try {
    ...
    res.status(200).json({ cellArray, cellInfo });
  } catch (error) {
    logger.error(error, { maintenanceType: maintenanceType?.[0] });
    console.log(error);
    console.log("User id not received!!!");
...
router.post("/postCellToGetLineList", authenticate, async (req, res) => {
  try {
    ...
    res.status(200).json({ lineArray, lineInfo: LineInfo });
  } catch (error) {
    logger.error(error, { maintenanceType: maintenanceType?.[0] });
    console.log(error);
    console.log("User id not received!!!");
...
router.post("/postLineToGetMachineList", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ machineArray, machineInfo, machineInfoWithChecksheet });
  } catch (error) {
    logger.error(error, { maintenanceType: maintenanceType?.[0] });
    console.log(error);
    console.log("User id not received!!!");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(200).json({ sectionArray: sectionArray });
    } catch (error) {
      logger.error(error, { maintenanceType: maintenanceType?.[0] });
      console.log(error);
      console.log("User id not received!!!");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(200).json({ subSectionArray: subSectionArray });
    } catch (error) {
      logger.error(error, { maintenanceType: maintenanceType?.[0] });
      console.log(error);
      console.log("User id not received!!!");
  ...
  async (req, res) => {
    try {
      ...
      const groupCondition = {
        ...
        total_completed: {
          $sum: {
            $cond: [{ $eq: [keyForSelectedMonth, "Completed"] }, 1, 0],
          },
        },
        total_ongoing: {
          $sum: {
            $cond: [{ $eq: [keyForSelectedMonth, "Ongoing"] }, 1, 0],
          },
        },
        total_previous_pending: {
          $sum: {
        ...
        total_done_with_delay: previousMonth
          ? {
              $sum: {
                $cond: [
                  {
      ...
      // Extract all line names into an array
      const allLineNames = lineData.map((line) => line._id);

      const pipeline = [
        {
          $match: {
            line_names: { $in: allLineNames },
            // $or: selectedYearOfCheckSheet,
            isPM: "Yes",
          },
        },
        ...
        },
        {
          $project: {
            ...
            total_completed: {
              $add: [
                "$total_completed",
                { $ifNull: ["$total_done_with_delay", 0] },
              ],
            },
            total_ongoing: 1,
            total_previous_pending: 1,
      ...
      const pipeline1 = [
        {
          $match: {
            ...
            $or: [
              ...
              {
                subSection_names: undefined,
              },
              {
                subSection_names: null,
              },
            ],
            // "subSection_names" :  mongoose.Types.ObjectId('6322e5b1fdb4a3119153b9d9'),
            // $or: selectedYearOfCheckSheet,
            isPM: "Yes",
          },
        },
      ...
      const result = await Machine.aggregate(pipeline);
...
router.get("/fetchAllSummeryData", authenticate, async (req, res, next) => {
  try {
    ...
    // const monthData1 = await Machine.aggregate([
    //   {
    //     $match: {
    //       // subSection_names: mongoose.Types.ObjectId("6322e5b1fdb4a3119153b9d9"),
    //       isPM: "Yes",
              ...
              $cond: [
                {
                  $eq: [
                    {
                      $getField: {
                        field: req.query?.selectedMonth,
                        input: "$checkSheet_data.PMStatus",
                      },
                    },
                    "Completed",
                  ],
