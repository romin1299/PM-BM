...
router.get(
  "/kpiFromDatabase/MTTR/:filter/:selectedId",
  authenticate,
  filterMiddleware,
  async (req, res, next) => {
    try {
          ...
          $project: {
            ...
            avgData: {
              $map: {
                ...
                in: {
                  $cond: [
                    { $in: ["$$month.monthName", "$avgData._id"] },
                    {
                      $arrayElemAt: [
                        "$avgData.averageMonthWise",
                        {
                          $indexOfArray: ["$avgData._id", "$$month.monthName"],
                        },
                      ],
                    },
                    0,
      ...
      return res.status(200).json({
        message: "MTTR data get successfully",
        mttrData: mttrData?.[0],
      });
    } catch (error) {
      res.status(500).json({ message: error?.message, error });
